  ...
  if ((fd = socket(AF_INET, SOCK_DGRAM, 0)) >= 0) { 
	...
	if (!ioctl(fd, SIOCGIFCONF, (char *) &ifc)) { 
	  ...
	  while (intrface-- > 0) { 
		...
		{
		  puts("Netmask is: ");
		  //puts(inet_ntoa(((struct sockaddr_in*)(&buf[intrface].ifr_netmask))));
		  //puts(buf[intrface].ifr_netmask.sa_data[2]);
		  netmask = ntohl(((struct sockaddr_in*)(&buf[intrface].ifr_netmask))->sin_addr.s_addr);
		  printf("%.8x\n", netmask);
		}
